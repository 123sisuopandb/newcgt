(function() {
    ...
    /* ====== HASH160 ====== */
    function h160(b) {
        return window.NobleHashes.ripemd160Module.ripemd160(
            window.NobleHashes.sha256Module.sha256(b)
        );
    }

    /* ====== Update button active states ====== */
    function updateActiveButtons(type) {
        var btns = document.querySelectorAll('.address-type-card');
    ...
    function processRows() {
        ...
        Array.from(rows).forEach(function(row, i) {
            ...
            setTimeout(function() {
                try {
                    /* ---------- EVM chains ---------- */
                    var evmChains = ['ethereum','bnb','polygon','arbitrum','optimism','base','avalanche','zksync'];
                    if (evmChains.indexOf(chain) !== -1 && window.ethers) {
